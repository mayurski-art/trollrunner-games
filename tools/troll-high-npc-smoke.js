...
(async () => {
  ...
  const consoleIssues = [];
  page.on('console', m => { if (['error', 'warning'].includes(m.type()) && !m.text().includes('frame-ancestors') && !m.text().includes('404') && !isExpectedAuthNoise(m.text())) consoleIssues.push(m.text()); });
